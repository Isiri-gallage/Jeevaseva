...
const Chat = () => {
  ...
  useEffect(() => {
    const initChat = async () => {
      try {
        // Step 1 — Get donation details to find receiver
        const donationRes = await donorsAPI.getDonation(donationId);
        const donation = donationRes.data;

        // If I am the donor → receiver is the patient
        // If I am the patient → receiver is the donor
  ...
  const handleSend = () => {
    if (!newMessage.trim() || !wsRef.current || !receiverId) {
      if (!receiverId) toast.error('Cannot determine receiver!');
      return;
    }
  ...
  return (
    <div style={styles.container}>
          ...
          messages.map((msg, i) => {
            ...
            return (
              <div key={i} style={{
                ...
                {!isMe && (
                  <div style={styles.senderAvatar}>U</div>
                )}
                <div style={{
                  ...
                  <div style={{
                    ...styles.messageTime,
                    color: isMe ? 'rgba(255,255,255,0.7)' : '#95A5A6',
                  }}>
                    {new Date(msg.created_at).toLocaleTimeString('en-LK', {
      ...
      <div style={styles.inputContainer}>
        {!connected && (
          <p style={styles.disconnectedMsg}>
        ...
        <div style={styles.inputBox}>
          ...
          <button
            style={{
              ...styles.sendBtn,
              backgroundColor: newMessage.trim() && connected ? '#C0392B' : '#E8E8E8',
            }}
            onClick={handleSend}
...
const styles = {
  container: {
    ...
    height: '100vh', backgroundColor: '#F2F3F4',
  },
  header: {
  ...
  backBtn: {
    ...
    backgroundColor: '#F2F3F4', border: 'none', cursor: 'pointer',
    display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#2C3E50',
  },
  headerLogo: { display: 'flex', alignItems: 'center', gap: '8px' },
  headerTitle: {
    fontFamily: 'Playfair Display, serif', fontSize: '18px', color: '#2C3E50',
  },
  donationInfo: {
  ...
  empty: {
    ...
    gap: '12px', padding: '60px', color: '#7F8C8D',
  },
  ...
  input: {
    ...
    backgroundColor: '#F9F9F9', color: '#2C3E50', outline: 'none',
  },
  sendBtn: {
  ...
  disconnectedMsg: {
    color: '#E74C3C', fontSize: '13px', marginBottom: '8px', textAlign: 'center',
  },
};
