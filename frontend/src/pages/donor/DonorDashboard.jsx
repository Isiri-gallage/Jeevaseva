...
const DonorDashboard = () => {
  ...
  const handleToggleAvailability = async () => {
    ...
    try {
      ...
      toast.success(user.is_available ? 'You are now unavailable' : 'You are now available! ✅');
    } catch {
    ...
    <Layout>
      ...
      <div style={styles.header}>
        ...
        <div style={{
          ...
          <div>
            <div style={{ ...styles.availabilityStatus, color: user?.is_available ? '#27AE60' : '#C0392B' }}>
              {user?.is_available ? 'Available to Donate' : 'Unavailable'}
            </div>
            <div style={styles.availabilityHint}>Toggle your availability</div>
          </div>
          <button
            style={{ ...styles.toggleBtn, backgroundColor: user?.is_available ? '#27AE60' : '#C0392B' }}
            onClick={handleToggleAvailability}
            disabled={togglingAvailability}
          >
            {user?.is_available ? <ToggleRight size={28} color="white" /> : <ToggleLeft size={28} color="white" />}
          </button>
        </div>
      </div>
      ...
      <div style={styles.statsGrid}>
        {[
          { label: 'Matching Requests', value: matchingRequests.length, icon: <Droplets size={24} color="#C0392B" />, color: '#C0392B', bg: '#FADBD8' },
          { label: 'My Donations', value: donations.length, icon: <Heart size={24} color="#27AE60" />, color: '#27AE60', bg: '#EAFAF1' },
          { label: 'Completed', value: donations.filter(d => d.status === 'completed').length, icon: <CheckCircle size={24} color="#2980B9" />, color: '#2980B9', bg: '#EBF5FB' },
          { label: 'Pending', value: donations.filter(d => d.status === 'pending').length, icon: <Clock size={24} color="#F39C12" />, color: '#F39C12', bg: '#FEF9E7' },
        ].map((stat, i) => (
      ...
      <div style={styles.section}>
        <div style={styles.sectionHeader}>
          ...
          <button style={styles.viewAllBtn} onClick={() => navigate('/matching-requests')}>
            View All
          </button>
        </div>
            ...
            {matchingRequests.slice(0, 3).map(request => (
              <div key={request.id} style={styles.requestCard}>
                ...
                <div style={styles.details}>
                  ...
                  <div style={styles.detailRow}><Droplets size={14} color="#C0392B" /><span style={{ color: '#C0392B', fontWeight: '500' }}>{request.units_needed} unit(s) needed</span></div>
...
const styles = {
  ...
  availabilityCard: { display: 'flex', alignItems: 'center', gap: '16px', padding: '16px 24px', borderRadius: '12px', border: '2px solid' },
  availabilityStatus: { fontSize: '15px', fontWeight: '600' },
  availabilityHint: { fontSize: '12px', color: '#7F8C8D', marginTop: '2px' },
  toggleBtn: { border: 'none', borderRadius: '8px', padding: '6px', cursor: 'pointer', display: 'flex', alignItems: 'center' },
  ...
  viewAllBtn: { padding: '8px 16px', backgroundColor: '#FADBD8', color: '#C0392B', borderRadius: '8px', fontSize: '14px', fontWeight: '500', cursor: 'pointer', border: 'none', fontFamily: 'DM Sans, sans-serif' },
  loading: { textAlign: 'center', padding: '40px', color: '#7F8C8D' },
  empty: { textAlign: 'center', padding: '40px', color: '#7F8C8D', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '12px' },
  requestsGrid: { display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '20px' },
  requestCard: { backgroundColor: 'white', borderRadius: '16px', padding: '24px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' },
  ...
  patientName: { fontSize: '18px', fontFamily: 'Playfair Display, serif', color: '#2C3E50', marginBottom: '12px' },
  ...
  respondBtn: { display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', width: '100%', padding: '12px', backgroundColor: '#C0392B', color: 'white', borderRadius: '10px', fontSize: '15px', fontWeight: '500', cursor: 'pointer', border: 'none', fontFamily: 'DM Sans, sans-serif' },
};
